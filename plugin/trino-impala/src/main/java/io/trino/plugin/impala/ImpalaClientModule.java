        ...
        implements Module
{
    ...
    {
        binder.bind(JdbcClient.class).annotatedWith(ForBaseJdbc.class).to(ImpalaClient.class).in(Scopes.SINGLETON);
        configBinder(binder).bindConfig(ImpalaJdbcConfig.class);
        configBinder(binder).bindConfig(ImpalaConfig.class);
    ...
    @Singleton
    @ForBaseJdbc
    public static ConnectionFactory createConnectionFactory(BaseJdbcConfig config, CredentialProvider credentialProvider, ImpalaConfig mySqlConfig)
            throws SQLException
    {
